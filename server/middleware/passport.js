    ...
    passport.use('login', new LocalStrategy(function(username   , password, done) {
        try {
            ...
            mongo.connect(mongoURL, function(){
                ...
                collection.find({email : username} , {email : 1 , fname : 1 , lname : 1 , gender : 1 , password : 1 , dob : 1  } ).toArray(function(err , result){
                	console.log(result[0]); 
                	if(result[0]){
                		bcrypt.compare(password, result[0].password, function(err, result1) {
                    		if(result1 == true){
                    			done(null, {user_id : result[0]._id , email : result[0].email , 
